...
На создание данного онлайн туллинга, меня воодушевили [HTML Academy](https://htmlacademy.ru/), а точнее их стремление к чистоте верстки и следованию строгим стандартам. В курсе по HTML верстке от [HTML Academy](https://htmlacademy.ru/), возможность вложения одного тега в другой, объясняется путем показа двух скриншотов со [страницы спецификации](https://html.spec.whatwg.org/), которые содержат информациею по тегам. Это не очень удобно для восприятия. И мне пришла в голову идея - упростить доступ к информации о тегах и придумать простой механизм, по детекту возможности вложения одного тега в другой. Далее, расскажу историю о создании онлайн туллинга `caninclude`.

## /Технологии/

Сразу встал вопрос, какие технологии использовать, а главное на каком хостинге разварачивать проект.
Хотелось чего-то легкого и незатейлевого, без использования бабелей, парцелей и вебпаков, в душе начал витать дух свободы и бунтарства. 
...
## /Реализация/
Мне понадобилось пару дней на то, чтобы пройти от генерации идеи до ее реализации. Так как я ни разу не дизайнер, взаимствовавал идеи от уже давно существующих веб-сайтов. В итоге, получился простой и емкий, в плане отображения информации, интерфейс.

В ходе фазы скреппинга, увидел дополнительную информацию с сайтов [caniuse.com](https://caniuse.com/) и [MDN](https://developer.mozilla.org/) по каждому тегу, расположенную на странице [HTML Spec WHATWG](https://html.spec.whatwg.org/), в виде небольших баджиков. Решил разместить информацию по поддержке тегов разными браузерами сразу под блоком с информацией, по сравниваемым тегам, на странице **caninclude** сравнения тегов. Чтобы, как говорится - "все было под рукой, в развернутом виде".
...
Для принятия решения, о возможности вложения одного тега в другой, необходимо сопоставить содержание секций сравнимаемых тегов. 

* У вкладываемого тега, нужно проверить секцию **Categories**, а у тега, в который предпологается вложение - секцию **Content model**.
...
* В случае с **Transparent** моделью в секции **Content model** родительского тега, в который предполагается включение **дочернего** тега, нужно обратить внимание на **Content model** вышестоящего тега от **родительского** тега в вашей верстке. Нужно обратиться к вашей разметке и взять имя тега, который включает в себя текущий **родительский** тег с **Transparent** моделью в секции **Content model** и произвести дополнительный запрос на странице `caninclude`, изменив в правом текстовом поле значение тега на новый, чтобы уточнить возможность включения тега из левого текcтового поля, в новый **родительский** тег, в правом текстовом поле.
...
3) Если присутсвует модель **Transparent** в секции **Content model** родительского тега, то ситуация с включением тега - **не определена**. В этом случае выдается предупреджающее сообщение, о том, что нужно заглянуть в разметку и взять тег, вышестоящий для текущего **родительского тега**, а затем повторить запрос. При этом пункт **4.** не выполняется.
