...
- Chrome, Safari и Firefox поддерживают `oklch()` и `oklab()`;
...
У OKLCH предсказуемая яркость — поэтому нам легко сделать цвет интерфейса из произвольного цвета, который задаёт пользователь (и сохранить читаемость текста). Посмотрите, как выбранный цвет влияет на цвета чекбоксов в [цветовом миксере](https://oklch.com/#70,0.1,319,100) Злых марсиан:
...
И здесь есть хорошая новость: [stylelint-gamut](https://github.com/fpetrakov/stylelint-gamut) может автоматически определять все P3-цвета которые нужно обернуть медиавыражением.

## Внедрение OKLCH в проект

### Шаг 1: Добавление полифила OKLCH в CSS

К марту 2023, `oklch()` [поддерживают](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/oklch) Chrome и Safari. Для стабильного результата в старых версиях у нас есть полифилы для статичных значений (без кастомных свойств).
