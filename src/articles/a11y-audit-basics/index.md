...
tags:
    - a11y
    - testing
preview: 'Узнаете, как аудит поможет начать улучшать доступность, о его типах, методологиях, пользовательских исследованиях и жизни после аудита.'
hero:
...
Статья пригодится тем, кто думает об улучшении доступности, но не знает, с чего начать. Из первой части узнаете, что такое аудит, каким он бывает и как выглядит в теории процесс оценки. Во второй познакомитесь с проведением самостоятельного аудита на практике.
...
- Расширение аудитории и улучшение пользовательского опыта. Доступным интерфейсом сможет пользоваться больше людей, а [71 % пользователей с особыми потребностями](https://devblogs.microsoft.com/startups/how-does-accessibility-fit-into-an-mvp/) не уйдёт с недоступного сайта сразу же. К тому же доступность влияет на положение в поисковой выдаче Google.
- Узнать, с чего лучше начать внедрение доступности, если компания решила за это взяться.
- Принятие решения об изменениях в интерфейсе. Например, когда разрабатываете новую функциональность.
- Уточнение состояния доступности, если интерфейс изменился. Актуально для больших проектов, в которых уже поддерживается определённый уровень доступности.
- Проверка соответствия законодательным требованиям. Особенно актуально в США и странах Европейского Союза. Там компании могут получить штраф или потратить деньги на судебные разбирательства из-за нарушения законов.
- Участие в госзакупках для компаний из США. Доступность — одно из требований для выигрыша тендера.
...
- Определяет уровень доступности в целом. Например, можно проверить общий уровень доступности сайта.
- Основывается на чёткой методологии.
- Включает так называемые _[контрольные точки доступности](https://www.boia.org/wcag-2.1-a/aa-principles-and-checkpoints) (Accessibility Checkpoints)_. Это проверка выполнения конкретных требований руководств или законов. Например, во [WCAG 2.1](https://www.w3.org/TR/WCAG21/) (Web Content Accessibility Guidelines, Руководство по обеспечению доступности веб-контента) контрольная точка — выполнен или нет критерий успешности.

Критерии для аудита берутся из международных и локальных руководств, стандартов и законов. Это могут быть:

- WCAG 2. Сейчас актуальна версия WCAG 2.1, а скоро [WCAG 2.2](https://www.w3.org/TR/WCAG22/).
...
<figure>
    <img src="images/1.png" alt="Книжная иллюстрация. Справа ветка с вишней и листьями. Слева утверждения о том, что такое WCAG.">
...
Чем раньше проводите аудит, тем лучше. Так избежите лишних трат на разработку и дизайн, а ещё уменьшите количество барьеров для пользователей. Например, можно провести его на первых этапах разработки, в процессе редизайна или при переходе на новые технологии.
...
Пока нет универсальной классификации типов аудита. Разные компании даже могут называть их по-разному. В этом разделе собрали самые распространённые типы и сгруппировали их так, чтобы было проще в них разобраться и выбрать походящий.
...
Для самостоятельного аудита создаётся отдельная рабочая группа внутри компании. Она состоит из инициатора аудита, который за ним следит, и нескольких аудиторов. Их количество зависит от объёма продукта. Максимально объективную оценку они смогут дать, если в этот момент активно не участвуют в разработке.
...
- Проверочный аудит (Validation Audit). Повторный аудит продукта, который уже был до этого проверен.

### По степени автоматизации

Тестирование — важный этап аудита. В этой категории типы выделены на основе того, какой вид тестирования преобладает.

#### Автоматический аудит (Software/Automated Audit)
Проводится с помощью автоматических инструментов и автотестов. Помогает быстро выявить основные проблемы с доступностью.
...
Так что начинать оценку хорошо с автоматического аудита, но не сто́ит на нём останавливаться.

#### Ручной аудит (Manual Audit)
Заполняет пробелы автоматического. Можно проверить интерфейс с клавиатуры, посмотреть страницу с экранной лупой или провести юзабилити-тестирование с пользователями вспомогательных технологий.

#### Смешанный аудит (Hybrid Audit)
Совмещает ручные и автоматические методы. С ним легче подстроить процесс тестирования под нужды компании и избежать сложностей и пробелов только ручного или автоматического тестирования.
...
- Какая цель у аудита?
    ...
    - Или что-то другое.
- Что это за продукт? Может быть, это сайт, веб-приложение или интерфейс, которым можно управлять голосом.
...
- Сколько можете выделить на это денег?
...
#### Шаг 3. Подбор репрезентативной выборки
...
В странах Европейского Союза заявление о доступности обязательно должно быть у сайтов и мобильных приложений государственных органов. Для него разработана [отдельная европейская модель](https://eur-lex.europa.eu/eli/dec_impl/2018/1523/oj).
...
- [Отчёт Gutenberg](https://www.wpcampus.org/blog/2019/05/gutenberg-audit-results/) — редактор контента от WordPress.
