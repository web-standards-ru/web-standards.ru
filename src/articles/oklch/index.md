...
При использовании одинакового пространства есть разные способы задать в нём точку. Oklab и lab существуют в декартовых координатах с осями `a` и `b` (`a`— от красного до зелёного, `b` — от голубого до желтого). OKLCH и LCH используют полярные координаты, где есть угол оттенка и расстояние для насыщенности.
